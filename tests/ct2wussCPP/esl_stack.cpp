...
esl_stack_ICreate(void)
{
  ...
  //ESL_ALLOC(ns, sizeof(ESL_STACK));
    ns = (ESL_STACK *) malloc(sizeof(ESL_STACK));
  ...
  //ESL_ALLOC(ns->idata, sizeof(int) * ns->nalloc);
    ns->idata = (int *) malloc(sizeof(int) * ns->nalloc);
  return ns;
...
esl_stack_CCreate(void)
{
  ...
  //ESL_ALLOC(cs, sizeof(ESL_STACK));
    cs = (ESL_STACK *) malloc(sizeof(ESL_STACK));
  ...
  //ESL_ALLOC(cs->cdata, sizeof(char) * cs->nalloc);
    cs->cdata = (char *) malloc(sizeof(char) * cs->nalloc);
  return cs;
...
esl_stack_PCreate(void)
{
  ...
  // ESL_ALLOC(ps, sizeof(ESL_STACK));
    ps = (ESL_STACK *) malloc(sizeof(ESL_STACK));
  ...
  //ESL_ALLOC(ps->pdata, sizeof(void *) * ps->nalloc);
    ps->pdata = (void **) malloc(sizeof(void *) * ps->nalloc);
  return ps;
...
esl_stack_IPush(ESL_STACK *ns, int x)
{
  ...
  if (ns->n == ns->nalloc) {
    //ESL_RALLOC(ns->idata, ptr, sizeof(int) * ns->nalloc * 2);
      if(ns->idata == NULL)
          ptr = (int *) malloc(sizeof(int) * ns->nalloc * 2);
      else
          ptr = (int *) realloc(ns->idata, sizeof(int) * ns->nalloc * 2);
      if(ptr != NULL)
          ns->idata = ptr;
...
esl_stack_CPush(ESL_STACK *cs, char c)
{
  ...
  if (cs->n == cs->nalloc) {
    //ESL_RALLOC(cs->cdata, ptr, sizeof(char) * cs->nalloc * 2);
      if(cs->cdata == NULL)
          ptr = (char *) malloc(sizeof(char) * cs->nalloc * 2);
      else
          ptr = (char *) realloc(cs->cdata, sizeof(char) * cs->nalloc * 2);
      if(ptr != NULL)
          cs->cdata = ptr;
...
esl_stack_PPush(ESL_STACK *ps, void *p)
{
  ...
  if (ps->n == ps->nalloc) {
    //ESL_RALLOC(ps->pdata, ptr, sizeof(void *) * ps->nalloc * 2);
      if(ps->pdata == NULL)
          ptr = (void **) malloc(sizeof(void *) * ps->nalloc * 2);
      else
          ptr = (void **) realloc(ps->pdata, sizeof(void *) * ps->nalloc * 2);
      if(ptr != NULL)
          ps->pdata = (void **)ptr;
...
esl_stack_Convert2String(ESL_STACK *cs)
{
  ...
  if (cs->n == cs->nalloc) 
    ...
    if(s == NULL)
        tmp = (char *) malloc(sizeof(char) * (cs->nalloc +1));
    else
        tmp = (char *) realloc(s, sizeof(char) * (cs->nalloc +1));
    if(tmp != NULL)
        s = (char *) tmp;
